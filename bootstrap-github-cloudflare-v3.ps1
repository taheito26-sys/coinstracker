...
function Test-WorkerStatus {
    ...
    $pricesUrl = "$WorkerBaseUrl/api/prices"
    ...
    if ($response.ok -ne $true) {
        Write-Warn "Worker responded but cache is not ready yet. Warming price cache via $pricesUrl"
        try {
            $null = Invoke-RestMethod -Method Get -Uri $pricesUrl -TimeoutSec 60
            Start-Sleep -Seconds 2
            $response = Invoke-RestMethod -Method Get -Uri $statusUrl -TimeoutSec 30
        }
        catch {
            throw "Worker health endpoint responded, but cache warmup failed. $($_.Exception.Message)"
        }
    }

    if ($response.ok -ne $true) {
        Write-Warn "Worker is reachable, but the status payload still reports ok = $($response.ok)."
        return
    }

    Write-Info "Worker health check passed."
}
...
function Try-Install-GitHubCli {
    param()
    ...
    if (-not $AutoInstallGh) {
        return $false
    }

    Write-Warn "GitHub CLI was not found. Attempting automatic installation."

    if (Get-Command winget -ErrorAction SilentlyContinue) {
        try {
            Invoke-External -FilePath winget -Arguments @("install", "--id", "GitHub.cli", "-e", "--accept-package-agreements", "--accept-source-agreements") -AllowFailure | Out-Null
        }
        catch {
            Write-Warn "winget install for GitHub CLI did not complete successfully."
        }
    }
    elseif (Get-Command choco -ErrorAction SilentlyContinue) {
        try {
            Invoke-External -FilePath choco -Arguments @("install", "gh", "-y") -AllowFailure | Out-Null
        }
        catch {
            Write-Warn "Chocolatey install for GitHub CLI did not complete successfully."
        }
    }
    else {
        Write-Warn "Neither winget nor choco was found, so the script cannot auto-install GitHub CLI."
    }
    ...
    foreach ($candidate in $possibleGh) {
...
function Show-ManualGitHubNextSteps {
    param(
        [string]$WorkingDirectory,
        [string]$Owner,
        [string]$Repo
    ...
    Write-Host ""
    Write-Host "Manual GitHub next steps:" -ForegroundColor Green
    Write-Host "1. Create a new empty GitHub repository in the browser."
    if (-not [string]::IsNullOrWhiteSpace($Owner) -and -not [string]::IsNullOrWhiteSpace($Repo)) {
        Write-Host "   Suggested repo URL: https://github.com/$Owner/$Repo"
    }
    Write-Host "2. In PowerShell, run these commands from the repo folder:"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "3. Then connect that GitHub repo to Cloudflare Pages."
}
...
if (-not $SkipGitHub) {
    ...
    if (-not $ghAvailable) {
        $ghAvailable = Try-Install-GitHubCli
    }

    if (-not $ghAvailable) {
        Write-Warn "GitHub CLI (gh) is not installed, so the script will continue in manual GitHub mode."
        Write-Warn "To automate GitHub repo creation next time, install GitHub CLI first or rerun with -AutoInstallGh."
        $SkipGitHub = $true
    }
}
...
if (-not $SkipGitHub) {
    ...
    Write-Warn "Skipping GitHub repo creation and push."
    Show-ManualGitHubNextSteps -WorkingDirectory $RepoPath -Owner $GitHubOwner -Repo $GitHubRepo
}
...
if ($SkipGitHub) {
    Write-Host "GitHub creation was not automated in this run. Follow the manual GitHub steps shown above, then connect the repo in Cloudflare Pages." -ForegroundColor Yellow
}
else {
